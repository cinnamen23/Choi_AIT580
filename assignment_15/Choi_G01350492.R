...
titles <- html_nodes(webpage, ".css-xdandi h3")
title_without_tags <- gsub("<.*?>", "", titles) 
print(title_without_tags)

news_summary <-html_nodes(webpage, "div p")
news_summary_without_tags <- gsub("<.*?>", "", news_summary) 
print(news_summary_without_tags)
...
write.csv(title_without_tags,file="NYT_titles.csv")
